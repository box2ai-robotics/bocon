...
# for joycon
from bocon.kinematics import Robot_IK, Robot_FK, get_robot
from bocon import BoCon
import math
...
xml_path = "./bocon/examples/scene.xml"
...
init_gpos = Robot_FK(init_qpos[1:5], robot=robot)
...
robot_controller = BoCon(device="right", 
...
try:
    with mujoco.viewer.launch_passive(mjmodel, mjdata) as viewer:
        start = time.time()
        # 初始化时强制设置相机朝向
        viewer.cam.azimuth = 230   # 水平旋转270度（正后方）
        viewer.cam.elevation = -20 # 俯仰角-30度（略微仰视）
        viewer.cam.distance = 1.0  # 相机距离目标2米
        viewer.cam.lookat = [0, -0.2, 0.3]  # 注视点位置（机械臂中心）
        ...
        while viewer.is_running() and time.time() - start < 1000:
            ...
            t = t + 1
            target_pose, gripper_state_r, _ = robot_controller.get_control()# update()
            ...
            y_r = 0.01
            pitch_r = -pitch_r 
            roll_r = roll_r - math.pi/2 # Robot末端旋转90度
            
            right_target_gpos = np.array([x_r, y_r, z_r, roll_r, pitch_r, 0.0])
            
            qpos_inv,IK_success = Robot_IK(mjdata.qpos[qpos_indices][1:5], right_target_gpos, robot=robot)
            ...
            if IK_success:
                ...
                robot_controller.set_position = right_target_gpos[0:3]
