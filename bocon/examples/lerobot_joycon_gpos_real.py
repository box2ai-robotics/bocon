...
# for joycon
from  bocon.kinematics import Robot_IK, Robot_FK, get_robot, feetech_arm
...
# init_gpos = np.concatenate((Robot_FK(init_qpos[1:5], robot=robot),))
...
try:
    ...
    with mujoco.viewer.launch_passive(mjmodel, mjdata) as viewer:
        ...
        while viewer.is_running() and time.time() - start < 1000:
            ...
            target_pose, gripper_state_r, _ = robot_controller.get_control()
            ...
            qpos_inv_mujoco, IK_success = Robot_IK(fd_qpos_mucojo, right_target_gpos, robot=robot)
