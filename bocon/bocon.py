...
class BoCon:
    def __init__(self, 
        ...
        if device == "right":
            self.bocon_id = get_R_id()
        elif device == "left":
            self.bocon_id = get_L_id()
        else:
            print("get a wrong device name of bocon")
        # print(f"detect {device} {self.bocon_id=}")
        if self.bocon_id[2][:6] != '9c:54:' and self.bocon_id != None:
            raise IOError("There is no bocon for robotics")
        
        # init bocon
        self.bocon = JoyCon(*self.bocon_id)
        
        self.gyro = GyroTrackingJoyCon(*self.bocon_id)
        self.lerobot = lerobot
        self.pitch_down_double = pitch_down_double
        ...
        self.button = ButtonEventJoyCon(*self.bocon_id, track_sticks=True)
        self.without_rest_init = without_rest_init
        # print(f"connect to {device} bocon successful.")
        
        print(f"\033[32mconnect to {device} bocon successful.\033[0m")
    ...
    def reset_bocon(self):
        ...
        self.orientation_sensor.reset_yaw()
        time.sleep(2)
        print(f"\033[32mBoCon calibrations is complete.\033[0m")
    ...
    def common_update(self):
        ...
        bocon_stick_v = self.bocon.get_stick_right_vertical() if self.bocon.is_right() else self.bocon.get_stick_left_vertical()
        if bocon_stick_v > 4000:
        ...
        bocon_stick_h = self.bocon.get_stick_right_horizontal() if self.bocon.is_right() else self.bocon.get_stick_left_horizontal()
        if self.horizontal_stick_mode == "y":
            if bocon_stick_h > 4000:
        ...
        bocon_button_up = self.bocon.get_button_r() if self.bocon.is_right() else self.bocon.get_button_l()
        if bocon_button_up == 1:
            if self.pure_xz:
        ...
        if not self.change_down_to_gripper:
            bocon_button_down = self.bocon.get_button_r_stick() if self.bocon.is_right() else self.bocon.get_button_l_stick()
        else:
            bocon_button_down = self.bocon.get_button_zr() if self.bocon.is_right() else self.bocon.get_button_zl
                        
        if bocon_button_down == 1:
            if self.pure_xz:
                self.position[2] -= 0.001 * self.dof_speed[2]
            else:
                self.position[0] -= 0.001 * self.direction_vector_up[0] * self.dof_speed[0] * self.direction_reverse[0]
        ...
        # Common buttons
        bocon_button_xup = self.bocon.get_button_x() if self.bocon.is_right() else self.bocon.get_button_up()
        bocon_button_xback = self.bocon.get_button_b() if self.bocon.is_right() else self.bocon.get_button_down()
        if bocon_button_xup == 1:
            self.position[0] += 0.001 * self.dof_speed[0]
        elif bocon_button_xback == 1:
            self.position[0] -= 0.001 * self.dof_speed[0]
        
        bocon_button_home = self.bocon.get_button_home() if self.bocon.is_right() else self.bocon.get_button_capture()
        if bocon_button_home == 1:
        ...
        for event_type, status in self.button.events():
            if (self.bocon.is_right() and event_type == 'plus' and status == 1) or (self.bocon.is_left() and event_type == 'minus' and status == 1):
                self.reset_button = 1
                self.reset_bocon()
            elif self.bocon.is_right() and event_type == 'a':
                self.next_episode_button = status
            elif self.bocon.is_right() and event_type == 'y':
                self.restart_episode_button = status
            elif ((self.bocon.is_right() and event_type == 'zr') or (self.bocon.is_left() and event_type == 'zl')) and not self.change_down_to_gripper:
                self.gripper_toggle_button = status
            elif ((self.bocon.is_right() and event_type == 'stick_r_btn') or (self.bocon.is_left() and event_type == 'stick_l_btn')) and self.change_down_to_gripper:
                self.gripper_toggle_button = status
    ...
    # More information
    def get_stick(self):
        stick_vertical = self.bocon.get_stick_right_vertical() if self.bocon.is_right() else self.bocon.get_stick_left_vertical()
        stick_horizontal = self.bocon.get_stick_right_horizontal() if self.bocon.is_right() else self.bocon.get_stick_right_horizontal()
        stick_button = self.bocon.get_button_r_stick() if self.bocon.is_right() else self.bocon.get_button_l_stick()
